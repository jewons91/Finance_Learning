    ...
    def step(self, actions):
        ...
        for i, (ticker, df) in enumerate(self.dfs.items()):
            if portfolio_return > 0.01:
                if actions[i] == 'Sell':
                    scaled_reward = portfolio_return * 15000  # 일정 수익률 이상일 때 보상 증가
                else:
                    scaled_reward = portfolio_return * 1500
            else:
...
# 트레이너 생성
trainer = PPOTrainer(env, policy, memory, optimizer, gamma=0.4, epsilon=0.8, epochs=10)
