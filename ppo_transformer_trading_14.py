...
class MultiStockTradingEnv(gym.Env):
    def __init__(self, dfs, stock_dim=3,input_dim=7, initial_balance=5000000, max_stock=10, seq_length=20):
    ...
    def ppo_update(self):
        ...
        for epoch in range(self.epochs):
            for batch in dataloader:
                batch_states, batch_actions, batch_old_logprobs, batch_returns, batch_advantages = [x.to(device) for x in batch]
                
                action_logprobs, state_values, dist_entropies = self.policy.evaluate(batch_states, batch_actions)
                total_logprobs = action_logprobs.sum(dim=1)
                total_entropies = dist_entropies.sum(dim=1)
                ratios = torch.exp(total_logprobs - batch_old_logprobs)
...
# 환경 생성
env = MultiStockTradingEnv(dfs, stock_dim=len(tickers), initial_balance=10000000, seq_length=20, input_dim=8)
...
# 트레이너 생성
trainer = PPOTrainer(env, policy, memory, optimizer, gamma=0.5, epsilon=0.8, epochs=10)
