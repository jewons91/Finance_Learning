...
for ticker in tickers:
    ...
    volume_cols = ['Volume']
    close_cols = ['Close']
    other_cols = ['Low', 'MA10', 'MA20', 'Upper_Band', 'Lower_Band']
    feature_cols = ['Close', 'Low', 'MA10', 'MA20', 'RSI', 'Upper_Band', 'Lower_Band', 'Volume']
    
    
    df.loc[:, close_cols] = standard_scaler.fit_transform(df[close_cols])
    
    close_mean = standard_scaler.mean_[0]
    close_scale = standard_scaler.scale_[0]
    
    df.loc[:, other_cols] = (df[other_cols] - close_mean) / close_scale
