...
class MultiStockTradingEnv(gym.Env):
    def __init__(self, dfs, features, stock_dim=3,input_dim=7, initial_balance=5000000, max_stock=500, seq_length=20):
    ...
    def _next_observation(self):
        ...
        for ticker, df in self.dfs.items():
            ...
            if idx < self.seq_length:
                # 충분한 시퀀스가 없으면 제로 패딩
                seq = df.loc[:idx, self.features].values
                ...
                # 시퀀스 슬라이싱
                seq = df.loc[idx - self.seq_length:idx - 1, self.features].values
    ...
    def step(self, actions):
        ...
        if portfolio_return > 0.01:
            scaled_reward = portfolio_return * 1500  # 일정 수익률 이상일 때 보상 증가
        else:
            scaled_reward = portfolio_return * 1000
...
class PPOTrainer:
    ...
    def ppo_update(self):
        ...
        for epoch in range(self.epochs):
            ...
            max_exp_value = 10
            ratios = torch.exp(total_logprobs - old_logprobs, max=max_exp_value)
    ...
    def plot_results(self, episode, total_reward, filename):
        ...
        for idx in sell_indices:
            plt.plot(idx, self.env.price_history['000660'][idx], 'ro', markersize=2)  # 빨간 점 찍기
...
for ticker in tickers:
    ...
    df = df.rename(columns={'Unnamed: 0': 'Time', 
                                ...
                                '고가': 'High',
                                '거래량': 'Total_Volume'})
    df['Power'] = df['Buy_Volume'] - df['Sell_Volume']
    ...
    df['Volume_Intensity'] = df['Buy_Volume'] - df['Sell_Volume']
    df['Volume_Intensity_Ratio'] = df['Volume_Intensity'] / (df['Total_Volume'] + 1e-5)  # 0으로 나누는 것 방지
    ...
    df.loc[:, price_features] = standard_scaler.fit_transform(df[price_features])
    df.loc[:, volume_features] = robust_scaler.fit_transform(df[volume_features])
...
# 환경 생성
env = MultiStockTradingEnv(dfs, stock_dim=len(tickers), features=features, initial_balance=100000000, seq_length=20, input_dim=len(features))

# PPO 모델, 옵티마이저 및 메모리 초기화
policy = ActorCritic([9] * env.stock_dim, seq_length=env.seq_length, input_dim=env.input_dim).to(device)
optimizer = optim.Adam(policy.parameters(), lr=1e-6)  # 학습률 감소
memory = Memory()

# 트레이너 생성
trainer = PPOTrainer(env, policy, memory, optimizer, gamma=0.5, epsilon=0.8, epochs=30)
