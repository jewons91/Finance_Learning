...
# TimesNet 모듈 정의 (8개의 TransformerEncoderLayer 스택)
...
class MultiStockTradingEnv(gym.Env):
    def __init__(self, dfs, stock_dim=3,input_dim=7, initial_balance=5000000, max_stock=500, seq_length=20):
        ...
        self.observation_space = spaces.Box(
            low=-np.inf, high=np.inf, shape=(self.stock_dim * (self.input_dim * self.seq_length + 1) + 1,), dtype=np.float32
        )
        self.action_space = spaces.MultiDiscrete([11] * self.stock_dim)  # 각 종목별로 11개의 액션
    ...
    def _next_observation(self):
        ...
        for ticker, df in self.dfs.items():
            ...
            if idx < self.seq_length:
                # 충분한 시퀀스가 없으면 제로 패딩
                seq = df.loc[:idx, ['Close', 'MA10', 'MA20', 'RSI', 'Volume', 'Upper_Band', 'Lower_Band']].values
    ...
    def step(self, actions):
        ...
        total_reward = 0
        invalid_action_penalty = -10000  # 불가능한 행동에 대한 페널티
        ...
        if portfolio_return > 0.01:
            scaled_reward = portfolio_return * 150  # 일정 수익률 이상일 때 보상 증가
        else:
...
class ActorCritic(nn.Module):
    def __init__(self,  action_dim_list, seq_length=20, input_dim=7):
        ...
        self.seq_length = seq_length
        self.input_dim = input_dim
        self.timesnet = TimesNet(input_dim=self.input_dim, seq_length=seq_length).to(device)  # input_dim=7 (Close, MA10, MA20, RSI, Volume, Bollinger Bands)
        self.policy_head = nn.ModuleList([nn.Linear(self.timesnet.embed_dim, action_dim) for action_dim in action_dim_list])
        self.value_head = nn.Linear(self.timesnet.embed_dim * len(action_dim_list), 1)  # embed_dim * stock_dim
    ...
    def forward(self, x):
        ...
        for i in range(len(tickers)):
            ...
            embed = self.timesnet(seq)  # [batch_size, embed_dim]
...
class PPOTrainer:
    ...
    def plot_results(self, episode, total_reward, file_name):
        # 행동 분포 및 결과 시각화
...
for ticker in tickers:
    ...
    feature_cols = ['Close', 'MA10', 'MA20', 'RSI', 'Upper_Band', 'Lower_Band']
    volume_cols = ['Volume']
    df.loc[:, feature_cols] = standard_scaler.fit_transform(df[feature_cols])
    df.loc[:, volume_cols] = robust_scaler.fit_transform(df[volume_cols])

    dfs[ticker] = df.reset_index(drop=True)

# 환경 생성
env = MultiStockTradingEnv(dfs, stock_dim=len(tickers), initial_balance=100000000, seq_length=20, input_dim=7)
...
# 트레이너 생성
trainer = PPOTrainer(env, policy, memory, optimizer, gamma=0.8, epsilon=0.8, epochs=10)
