...
class MultiStockTradingEnv(gym.Env):
    def __init__(self, dfs, stock_dim=3,input_dim=7, initial_balance=5000000, max_stock=500, seq_length=20):
        ...
        self.max_loss = 0.05  # 최대 허용 손실 (20%)
        ...
        self.observation_space = spaces.Box(
            low=-np.inf, high=np.inf, shape=(self.stock_dim * (self.input_dim * self.seq_length + 3) + 1,), dtype=np.float32
        )
        self.action_space = spaces.MultiDiscrete([9] * self.stock_dim)  # 각 종목별로 11개의 액션
    ...
    def reset(self):
        ...
        self.current_step = 0
        self.stock_owned = {ticker: {'quantity': 0, 'avg_price': 0} for ticker in self.copy_df.keys()}
        self.stock_price = {}
        self.total_asset = []
        ...
        self.action_history = []
        self.price_history = {ticker: [] for ticker in self.copy_df.keys()}
        self.trade_history = []  # 매수/매도 내역 저장

        # 각 종목의 최대 스텝 수 계산
        self.max_steps = min(len(df) for df in self.copy_df.values()) - self.seq_length - 1
        if self.max_steps <= 0:
            raise ValueError("데이터프레임의 길이가 시퀀스 길이보다 짧습니다.")

        # 각 종목의 현재 인덱스 초기화
        self.data_indices = {ticker: self.seq_length for ticker in self.copy_df.keys()}  # 시퀀스 길이만큼 초기 인덱스

        return self._next_observation()

    def _next_observation(self):
        obs = []
        for ticker, df in self.copy_df.items():
            idx = self.data_indices[ticker]
            if idx < self.seq_length:
                # 충분한 시퀀스가 없으면 제로 패딩
                seq = df.loc[:idx, ['Close', 'Low', 'MA10', 'MA20', 'RSI', 'Power', 'Upper_Band', 'Lower_Band']].values
                ...
                # 시퀀스 슬라이싱
                seq = df.loc[idx - self.seq_length:idx - 1, ['Close', 'Low', 'MA10', 'MA20', 'RSI', 'Power', 'Upper_Band', 'Lower_Band']].values
            ...
            obs.append(df.iloc[idx]['Close_unscaled'])
    ...
    def step(self, actions):
        ...
        total_reward = 0
        invalid_action_penalty = -100  # 불가능한 행동에 대한 페널티
        ...
        # 종목별로 행동 수행
        for i, (ticker, df) in enumerate(self.copy_df.items()):
            action = actions[i]
            idx = self.data_indices[ticker]
        ...
        self.portfolio_value = self.balance + sum(
            self.stock_owned[ticker]['quantity'] * self.stock_price[ticker] for ticker in self.copy_df.keys()
        )
        ...
        for i, (ticker, df) in enumerate(self.copy_df.items()):
            if portfolio_return > 0.01:
                scaled_reward = portfolio_return * 20000
            else:
...
class ActorCritic(nn.Module):
    ...
    def act(self, state):
        ...
        for logits in policy_logits:
            if env.stock_price['000660'] > env.balance or env.stock_owned['000660']['quantity'] >= 500: # 매수 불가 행동 제약
                logits = logits[0][:5]
            ...
            elif env.stock_owned['000660']['quantity'] == 0: # 매도 불가 행동 제약
                logits = logits[0][4:]
                dist = Categorical(logits=logits)
                action = dist.sample()
                actions.append(action.item()+4)
                action_logprob = dist.log_prob(action)
                action_logprobs.append(action_logprob)
            else:
                logits = logits
...
class PPOTrainer:
    ...
    def train(self, max_episodes, model_name):
        ...
        for episode in range(max_episodes):
            env.copy_df = env.dfs['000660'][1000*episode:1000*(episode+1)]
...
for ticker in tickers:
    ...
    feature_cols = ['Close', 'Low', 'MA10', 'MA20', 'RSI', 'Upper_Band', 'Lower_Band']
    power_cols = ['Power']
...
# 환경 생성
env = MultiStockTradingEnv(dfs, stock_dim=len(tickers), initial_balance=100000000, seq_length=20, input_dim=8)
...
# 트레이너 생성
trainer = PPOTrainer(env, policy, memory, optimizer, gamma=0.6, epsilon=0.8, epochs=10)

model_name = 'model_16'
# 학습 시작
trainer.train(max_episodes=1000, model_name=model_name)
