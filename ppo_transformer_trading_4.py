...
class MultiStockTradingEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        for i, (ticker, df) in enumerate(self.dfs.items()):
            ...
            if action_type == 'sell':
                if df.iloc[idx]['Close_unscaled'] > df.iloc[idx+1]['Close_unscaled']:
                    reward += 100
...
class PPOTrainer:
    ...
    def plot_results(self, episode, total_reward, filename):
        ...
        for idx in sell_indices:
            plt.plot(idx, self.env.portfolio_value_history[idx], 'ro', markersize=2)  # 빨간 점 찍기
...
# 트레이너 생성
trainer = PPOTrainer(env, policy, memory, optimizer, gamma=0.7, epsilon=0.8, epochs=10)
